...
> [!NOTE]
> The `fetch` implementation of `HttpClient` does not report upload progress events.
...
However, as with any asynchronous operation, we strongly recommend cleaning up subscriptions when the component using them is destroyed, as subscription callbacks may run and encounter errors when attempting to interact with a destroyed component.
