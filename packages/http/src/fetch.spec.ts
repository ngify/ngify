...
describe('FetchBackend', async () => {
  ...
  describe('gets response URL', async () => {
    it('from the response URL', () => new Promise<void>(done => {
      backend
        .handle(TEST_POST)
        .pipe(toArray())
        .subscribe((events) => {
    ...
    it('from X-Request-URL header if the response URL is not present', () => new Promise<void>(done => {
      backend
        .handle(TEST_POST)
        .pipe(toArray())
        .subscribe((events) => {
  ...
  describe('dynamic global fetch', () => {
    it('should use the current implementation of the global fetch', async () => {
      ...
      try {
        const fakeFetch = vitest.fn(() => Promise.resolve(new Response(JSON.stringify({ foo: 'bar' }))));
        globalThis.fetch = fakeFetch;

        const client = new HttpClient(withFetch());
        ...
        const fakeFetch2 = vitest.fn(() => Promise.resolve(new Response(JSON.stringify({ foo: 'baz' }))));
